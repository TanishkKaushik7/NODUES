...
// Animation variants (Keeping the Framer Motion variants)
...
const DepartmentDashboard = () => {
  ...
  // --- START: Merged state from LibraryDashboard
  const [actionDeptId, setActionDeptId] = useState(null);
  const [actionRemark, setActionRemark] = useState('');
  const [actionLoading, setActionLoading] = useState(false);
  const [actionError, setActionError] = useState('');
  // --- END: Merged state from LibraryDashboard

  const applicationPopupRef = useRef(null);

  // Define DEPARTMENTS (Copied from LibraryDashboard for consistency in action logic)
  const DEPARTMENTS = [
  ...
  // Helper: Render Status Badge (Copied from LibraryDashboard)
  const renderStatusBadge = (status) => {
    ...
    if (['inprogress', 'in_progress', 'pending'].includes(key)) { // Combined pending and in progress
      return (
      ...
      const allApplications = Array.isArray(enriched)
        ? enriched.map(app => {
            ...
            return {
              ...
              match: true, // For search functionality
            };
          })
        : [];

      setApplications(allApplications.filter(app => app.id)); // Only include records with an ID
    } catch (err) {
      setApplications([]);
  ...
  const handleDepartmentAction = async (application, departmentId, action, remarksIn) => {
    ...
    // Reset error on new action attempt
    setActionError('');
    ...
    if (!stageId) {
      setActionError('Missing stage id; cannot perform department action. Application ID: ' + appId);
      return;
    }
    ...
    const call = async (url, options) => {
      ...
      if (res && res.status === 405) {
        // Fallback for method not allowed (try POST if original was GET/PUT)
        try { res = await authFetch(url, { method: 'POST' }); } catch (_) {}
      }
      if (res && res.status === 401) {
        // Simple retry logic (minimal)
        try {
    ...
    try {
      ...
      // Update selected application state (for popup)
      ...
      // Clear action state and close popup on success
      setActionRemark('');
      ...
      // Re-show alert for user feedback
      alert(`Action failed: ${err?.message || String(err)}`);
    } finally {
      setActionLoading(false);
    }
  };
  // --- END: Application Fetch Logic

  // Close popup on outside click
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        applicationPopupRef.current &&
        !applicationPopupRef.current.contains(event.target)
      ) {
        setSelectedApplication(null);
        // Clear action states on close
        setActionRemark(''); 
        setActionError('');
  ...
  // *** FIX: Simplified hook logic to always use the logged-in user's department ID ***
  // Set initial actionDeptId to the user's department when the application changes
  useEffect(() => {
    if (selectedApplication) {
        setActionDeptId(user?.department_id || null);
    }
  }, [selectedApplication, user?.department_id]); // Depend on selectedApplication and user dept ID

  // Filter handling (now includes search matching)
  ...
  // --- START: Merged UI from LibraryDashboard (renderApplicationPopup)
  const renderApplicationPopup = () => {
    if (!selectedApplication) return null;

    // Get current user's department name for display
    const userDepartmentName = user?.department_name || ' Department';

    // Check if application is in an actionable state (Pending or In Progress)
          ...
          <div className="p-6">
            ...
            {isActionable && (
                ...
                <div className="space-y-4">
                    ...
                    {/* *** FIX: Display Current Department statically *** */}
                    <div>
                        ...
                        {/* The actual actionDeptId is managed by state and useEffect */}
                    </div>
                    {/* *** END FIX *** */}
            ...
            {/* Quick Actions (Keeping the LibraryDashboard style) */}
  ...
  // --- END: Merged UI from LibraryDashboard
        ...
        >
          {apps.map((app, index) => (
            ...
            >
              ...
              <td className="px-6 py-4">
                <button
                    // Modified onClick to reset action state before opening
                    onClick={() => {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      {/* LEFT SIDEBAR (Assuming Sidebar component exists) */}
            ...
            >
                ...
                <motion.div variants={itemVariants}>
                    <h1 className="text-3xl font-extrabold mb-1 text-gray-900">
                        {user?.department_name || 'Department'} Review
                    </h1>
