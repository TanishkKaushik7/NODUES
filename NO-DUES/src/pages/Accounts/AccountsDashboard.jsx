...
// Animation variants (Copied from Library/Hostel Dashboard)
...
const ACCOUNTS_DEPARTMENT_ID = 4; // Department ID for the Accounts department stage
...
const AccountsDashboard = () => {
  ...
  const menuItems = [ // RENAMED: from officeMenuItems to menuItems, paths updated for consistency
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      const allApplications = Array.isArray(enriched)
        ...
        : [];
      // Filter out records without an ID (which shouldn't happen but is a safety check)
  ...
  const handleDepartmentAction = async (application, departmentId, action, remarksIn) => {
    ...
    try {
      ...
      // Re-alerting user about the failure
...
    if (!selectedApplication) return null;
    ...
    const userDepartmentName = user?.department_name || 'Accounts'; // CHANGED: Default name to 'Accounts'
                ...
                <div className="space-y-4">
                    ...
                    <button
                        ...
                        onClick={() => handleDepartmentAction(
                            selectedApplication, 
                            ACCOUNTS_DEPARTMENT_ID, // Use fixed ID 4
                            'approve', 
                            actionRemark
                        )}
            ...
            >
                ...
                <motion.div variants={itemVariants}>
                    <h1 className="text-3xl font-extrabold mb-1 text-gray-900">
                        Accounts Review {/* CHANGED: Title */}
                    </h1>
                    <p className="text-gray-600 mb-6">Welcome, {user?.name}. Review and process pending No-Dues applications for Accounts.</p> {/* CHANGED: Subtitle */}
                </motion.div>
